...
public class UserAuthController {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<RsData<Void>> refreshToken(HttpServletRequest request, HttpServletResponse response) {
        boolean success = userAuthService.refreshTokens(request, response);

        if (success) {
            return ResponseEntity.ok(RsData.of(200, "토큰이 성공적으로 갱신되었습니다."));
        } else {
            return ResponseEntity.status(401)
                    .body(RsData.of(401, "토큰 갱신에 실패했습니다. 다시 로그인해주세요."));
        }
    }
    ...
    @PostMapping("/logout")
    public ResponseEntity<RsData<Void>> logout(HttpServletRequest request, HttpServletResponse response) {
        userAuthService.logout(request, response);
        return ResponseEntity.ok(RsData.of(200, "로그아웃되었습니다."));
    }
}